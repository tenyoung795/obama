; Super Obama Bros., a series of modifications to Super Mario Bros.
; Modifications by Ten-Young Guh, 2012
;
; Major character changes include:
;	Mario -> Obama
;	Luigi -> Biden
;	Bowser -> bin Laden
;
; Code changes were among the modifications made to Super Mario Bros.
; Some of the added or modified subroutines include:
;	WriteGameText: To print up to three digits for player lives
;	PrintPlayerHP: To print the player's HP to the status bar
;	PrintBinLadenHP: To print bin Laden's HP to the status bar
;	InitBinLaden: To increase bin Laden's HP by 5 for every world
;	Shroom_Flower_PUp: To support up to five HP for the player
;	ProcFireball_Bubble: To allow the player to shoot if and only if bin Laden is there
;	ChkFootMTile: To activate the axe only when bin Laden is killed
;	GetGfxOffsetAdder: To switch the graphics between Obama and Biden
;
...
;	Fireball gravity
;	Hammer gravity
;	Bridge destruction by axe
;
; The original disassembly was from a ROM hacker who goes under the handle of "doppelganger".
; It may be found at
;	http://www.romhacking.net/download/documents/344/
; His original introduction follows.
;
...
BinLadenBodyControls    = $0363
BinLadenFeetCounter     = $0364
BinLadenMovementSpeed   = $0365
BinLadenOrigXPos        = $0366
BinLadenFlameTimerCtrl  = $0367
BinLadenFront_Offset    = $0368
BinLadenGfxFlag         = $036a
BinLadenHitPoints       = $0483
BinLadenMaxHP	      = $070b
MaxRangeFromOrigin    = $06dc

BinLadenFlamePRandomOfs = $0417
...
VRAM_AddrTable_Low:
      ...
      .db <VRAM_Buffer2, <VRAM_Buffer2, <BinLadenPaletteData
      .db <DaySnowPaletteData, <NightSnowPaletteData, <MushroomPaletteData
      .db <ObamaThanksMessage, <BidenThanksMessage, <MushroomRetainerSaved
...
ThankPlayer:   tay                       ;put primary message counter into Y
               ...
               beq EvalForMusic          ;if obama, branch
               iny                       ;otherwise increment Y once for biden and
...
DecNumTimer:  dec FloateyNum_Timer,x       ;decrement value here
              ...
              lda NumberofLives            ;give player one extra life (1-up)
	      cmp #$ff
	      beq SoundExtraLife
	      inc NumberofLives
SoundExtraLife:	
...
BackgroundColors:
      .db $12, $21, $0c, $0f ;used by area type if bg color ctrl not set
      .db $03, $3c, $03, $0c ;used by background color control if set

PlayerColors:
      .db $0f, $30, $17, $08 ;obama's colors
      .db $0f, $30, $26, $2d ;biden's colors
...
GetPlayerColors:
               ...
               beq StartClrGet
               ldy #$04                 ;load offset for biden
StartClrGet:   lda #$03                 ;do three colors
               sta $00
...
GameText:
TopStatusBarLine:
  .db $20, $43, $05, $18, $0b, $0a, $16, $0a	; "OBAMA"
...
TwoPlayerTimeUp:
  .db $21, $cd, $05, $18, $0b, $0a, $16, $0a	; "OBAMA"
OnePlayerTimeUp:
...
BidenName:
  .db $0b, $12, $0d, $0e, $17    ; "BIDEN", no address or length
...
ChkBiden:    lsr
             bcc ExitChkName        ;if obama is current player, do not change the name
             ldy #$04
NameLoop:    lda BidenName,y        ;otherwise, replace "OBAMA" with "BIDEN"
...
ColorRotatePalette:
       .db $27, $37, $27, $17, $07, $17

BlankPalette:
       .db $3f, $0c, $04, $0f, $0f, $0f, $0f, $00
...
Palette2_MTiles:
  ...
  .db $9f, $24, $9f, $24 ;water top (underwater)
  .db $b0, $b1, $b2, $b3 ;cloud level terrain
  .db $77, $79, $77, $79 ;binladen's bridge
...
;"THANK YOU BIDEN!"
  ...
  .db $1d, $11, $0a, $17, $14, $24
  .db $22, $18, $1e, $24
  .db $0b, $12, $0d, $0e, $17, $2b
  .db $00

MushroomRetainerSaved:
;"BUT THE FIRST LADY IS IN"
  .db $25, $c4, 24
  .db $0b, $1e, $1d, $24
  .db $1d, $11, $0e, $24
  .db $0f, $12, $1b, $1c, $1d, $24
  .db $15, $0a, $0d, $22, $24,
  .db $12, $1c, $24
  .db $12, $17
;"ANOTHER HIDEOUT!"
...
UpdateTopScore:
      ldx #$05          ;start with obama's score
      jsr TopScoreCheck
      ldx #$0b          ;now do biden's score
...
PrimaryGameSetup:
      ...
      lda #$02
      sta PlayerHitPoints         ;set player's hit points to 2
...
SetStPos: lda PlayerStarting_X_Pos,y  ;load appropriate horizontal position
          ...
          sta StarInvincibleTimer     ;clear star obama timer
...
ContinueGame:
           ...
           lda #$02                  ;actual world and area numbers, then
           sta PlayerHitPoints            ;reset player's size, status, and
...
ForeSceneryData:
   .db $88, $00, $00, $00, $00, $00, $00   ;in water
   .db $00, $00, $00, $00, $69, $69
...
TerMTile: lda TerrainMetatiles,y     ;otherwise get appropriate metatile for area type
          ...
          lda #$89                  ;use cloud block terrain
...
AxeObj:
      lda #$08                  ;load binladen's palette into sprite portion of palette
...
BrickMetatiles:
      .db $22, $51, $52, $52
      .db $89 ;used only by row of bricks object
...
ProcELoop:    stx ObjectOffset           ;put incremented offset in X as enemy object offset
              ...
              bpl NoChgMus
              lda StarInvincibleTimer    ;if star obama invincibility timer at zero,
...
ProcFireball_Bubble:
      lda BinLadenHitPoints
      beq ProcAirBubbles         ;if no alive binladen, branch
      lda A_B_Buttons
...
SetHSpd:  lda Enemy_State,y          ;get enemy object state
...
SetBrosHY
	  pla
          clc
          adc #2                 ;move position 8 pixels upward
SetHY:	  
...
GiveOneCoin:
      ...
      lda NumberofLives      ;give the player an extra life
      cmp #$ff
      beq SndExtraLife
      inc NumberofLives
SndExtraLife:	
...
PlayerHeadCollision:
           ...
           lda #$12                 ;load breakable object state
...
PutMTileB: sta Block_Metatile,x     ;store whatever metatile be appropriate here
           ...
           beq BigBP                ;if not, branch to use default offset
...
MoveEnemySlowVert:
          ldy #$0f         ;set movement amount for binladen/other objects
...
EnemiesAndLoopsCore:
            ...
            pha                      ;save in stack
            asl
            bcs ChkBinLadenF           ;if MSB set in enemy flag, branch ahead of jumps
            pla                      ;get from stack
...
ChkBinLadenF: pla                      ;get data from stack
            and #%00001111           ;mask out high nybble
            tay
...
InitBinLaden:
      jsr DuplicateEnemyObj     ;jump to create another binladen object
      stx BinLadenFront_Offset    ;save offset of first here
      lda #$00
      sta BinLadenBodyControls    ;initialize binladen's body controls
      lda Enemy_X_Position,x
      sta BinLadenOrigXPos        ;store original horizontal position here
      lda #$df
      sta BinLadenFireBreathTimer ;store something here
      sta Enemy_MovingDir,x     ;and in moving direction
      lda #$20
      sta BinLadenFeetCounter     ;set binladen's feet timer and in enemy timer
...
MultiplyBinLadenHP:	
      ...
      adc #5
      dey
      bpl MultiplyBinLadenHP
      sta BinLadenHitPoints       ;give binladen 5 hit points
      sta BinLadenMaxHP
      txa
      pha
      tya
      ...
      tax
      lda #2
      sta BinLadenMovementSpeed   ;set default movement speed here
      rts
...
InitBinLadenFlame:
        ...
        lda NoiseSoundQueue
        ora #Sfx_BinLadenFlame        ;load binladen's flame sound into queue
        sta NoiseSoundQueue
        ldy BinLadenFront_Offset      ;get binladen's buffer offset
        lda Enemy_ID,y              ;check for binladen
        cmp #BinLaden
        beq SpawnFromMouth          ;branch if found
        jsr SetFlameTimer           ;get timer data based on flame counter
        clc
        adc #$60                    ;add 32 frames by default
        ldy SecondaryHardMode
        beq SetFrT                  ;if secondary mode flag not set, use as timer setting
        sec
        sbc #$30                    ;otherwise subtract 16 frames for secondary hard mode
SetFrT: sta FrenzyEnemyTimer        ;set timer accordingly
        lda PseudoRandomBitReg,x
        and #%00000011              ;get 2 LSB from first part of LSFR
        sta BinLadenFlamePRandomOfs,x ;set here
        tay                         ;use as offset
...
SpawnFromMouth:
       lda Enemy_X_Position,y    ;get binladen's horizontal position
       sec
       sbc #$0e                  ;subtract 14 pixels
       sta Enemy_X_Position,x    ;save as flame's horizontal position
       lda Enemy_PageLoc,y
       sta Enemy_PageLoc,x       ;copy page location from binladen to flame
       lda Enemy_Y_Position,y
       clc                       ;add 8 pixels to binladen's vertical position
       adc #16
...
JmpEO: jsr JumpEngine
      ...
      .dw RunBinLadenFlame    ;for objects $15-$1f
      .dw RunFireworks
      .dw NoRunCode
      .dw NoRunCode
      .dw NoRunCode
...
FirebarCollision:
         ...
         lda StarInvincibleTimer  ;if star obama invincibility timer
...
RunBinLaden:
      ...
      and #%00100000          ;then branch elsewhere to run binladen
      beq BinLadenControl
      lda Enemy_Y_Position,x  ;otherwise check vertical position
      cmp #$e0                ;if above a certain point, branch to move defeated binladen
      bcs KillAllEnemies        ;otherwise proceed to KillAllEnemies

MoveD_BinLaden:
       jsr MoveEnemySlowVert     ;do a sub to move binladen downwards
       jmp BinLadenGfxHandler      ;jump to draw binladen's front and rear, then leave
...
ChkMouth:  lda BinLadenBodyControls     ;check binladen's mouth
           bpl FeetTmr                ;if bit clear, go ahead with code here
           jmp HammerChk              ;otherwise skip a whole section starting here
FeetTmr:   dec BinLadenFeetCounter      ;decrement timer to control binladen's feet
           bne ResetMDr               ;if not expired, skip this part
           lda #$20                   ;otherwise, reset timer
           sta BinLadenFeetCounter        
           lda BinLadenBodyControls     ;and invert bit used
           eor #%00000001             ;to control binladen's feet
           sta BinLadenBodyControls
...
B_FaceP:   lda EnemyFrameTimer,x      ;if timer set here expired,
           ...
           jsr PlayerEnemyDiff        ;get horizontal difference between player and binladen,
           bpl GetPRCmp               ;and branch if binladen to the right of the player
           lda #$01
           sta Enemy_MovingDir,x      ;set binladen to move and face to the right
           lda #$02
           sta BinLadenMovementSpeed    ;set movement speed
           lda #$20
           sta EnemyFrameTimer,x      ;set timer here
           sta BinLadenFireBreathTimer  ;set timer used for binladen's flame
           lda Enemy_X_Position,x        
           cmp #$c8                   ;if binladen to the right past a certain point,
...
GetPRCmp:  lda FrameCounter           ;get frame counter
           ...
           lda Enemy_X_Position,x
           cmp BinLadenOrigXPos         ;if binladen not at original horizontal position,
...
GetDToO:   lda Enemy_X_Position,x
           clc                        ;add movement speed to binladen's horizontal
           adc BinLadenMovementSpeed    ;coordinate and save as new horizontal position
           sta Enemy_X_Position,x
           ldy Enemy_MovingDir,x
           cpy #$01                   ;if binladen moving and facing to the right, skip ahead
           beq HammerChk
           ldy #$ff                   ;set default movement speed here (move left)
           sec                        ;get difference of current vs. original
           sbc BinLadenOrigXPos         ;horizontal position
...
SetHmrTmr: lda Enemy_Y_Position,x     ;get current vertical position
           ...
           lda PseudoRandomBitReg,x
           and #%00000010             ;get pseudorandom offset
	   ora #%00000101
           tay
...
MakeBJump: cmp #$01                   ;if timer not yet about to expire,
           ...
           lda #$fe
           sta Enemy_Y_Speed,x        ;set vertical speed to move binladen upwards
ChkFireB:  lda WorldNumber            ;check world number here
           cmp #World8                ;world 8?
           beq SpawnFBr               ;if so, execute this part here
           cmp #World6                ;world 6-7?
           bcs BinLadenGfxHandler       ;if so, skip this part here
SpawnFBr:  lda BinLadenFireBreathTimer  ;check timer here
           bne BinLadenGfxHandler       ;if not expired yet, skip all of this
           lda #$20
           sta BinLadenFireBreathTimer  ;set timer here
           lda BinLadenBodyControls
           eor #%10000000             ;invert binladen's mouth bit to open
           sta BinLadenBodyControls     ;and close binladen's mouth
           bmi ChkFireB               ;if binladen's mouth open, loop back
           jsr SetFlameTimer          ;get timing for binladen's flame
           ldy SecondaryHardMode
           beq SetFBTmr               ;if secondary hard mode flag not set, skip this
           sec
           sbc #4                   ;otherwise subtract from value in A
SetFBTmr:  sta BinLadenFireBreathTimer  ;set value as timer here
           lda #BinLadenFlame           ;put binladen's flame identifier
           sta EnemyFrenzyBuffer      ;in enemy frenzy buffer

;--------------------------------

BinLadenGfxHandler:
          jsr ProcessBinLadenHalf    ;do a sub here to process binladen's front
          ldy #$10                 ;load default value here to position binladen's rear
...
CopyFToR: tya                      ;move binladen's rear object position value to A
          clc
          adc Enemy_X_Position,x   ;add to binladen's front object horizontal coordinate
          ldy DuplicateObj_Offset  ;get binladen's rear object offset
          sta Enemy_X_Position,y   ;store A as binladen's rear horizontal coordinate
          lda Enemy_Y_Position,x
          clc                      ;add eight pixels to binladen's front object
          adc #$08                 ;vertical coordinate and store as vertical coordinate
          sta Enemy_Y_Position,y   ;for binladen's rear
          lda Enemy_State,x
          ...
          stx ObjectOffset
          lda #BinLaden              ;set binladen's enemy identifier
          sta Enemy_ID,x           ;store in binladen's rear object
          jsr ProcessBinLadenHalf    ;do a sub here to process binladen's rear
          pla
          sta ObjectOffset         ;get original enemy object offset
          tax
          lda #$00                 ;nullify binladen's front/rear graphics flag
          sta BinLadenGfxFlag
ExBGfxH:  rts                      ;leave!

ProcessBinLadenHalf:
      inc BinLadenGfxFlag         ;increment binladen's graphics flag, then run subroutines
      jsr RunRetainerObj        ;to get offscreen bits, relative position and draw binladen (finally!)
      lda Enemy_State,x
...
FlameTimerData:
      .db $14, $07, $14, $14, $14, $07, $07, $14

SetFlameTimer:
      ldy BinLadenFlameTimerCtrl  ;load counter as offset
      inc BinLadenFlameTimerCtrl  ;increment
      lda BinLadenFlameTimerCtrl  ;mask out all but 3 LSB
      and #%00000111            ;to keep in range of 0-7
      sta BinLadenFlameTimerCtrl
      sta BinLadenFlameTimerCtrl
...
SFlmX:   sta $00                     ;store value here
         ...
         sbc #$00                    ;subtract borrow from page location
         sta Enemy_PageLoc,x
         ldy BinLadenFlamePRandomOfs,x ;get some value here and use as offset
         lda FlameYPosData,y         ;compare against coordinate data using $0417,x as offset
	 ldy WorldNumber
	 cpy #World8
	 bne CompareFlameY
	 cmp #$88
	 ...
	 lda #$88
CompareFlameY:
	 cmp Enemy_Y_Position,x      ;load vertical coordinate
...
SetGfxF: jsr RelativeEnemyPosition   ;get new relative coordinates
         lda Enemy_State,x           ;if binladen's flame not in normal state,
...
FlmeAt:  sty $01                     ;set binladen's flame sprite attributes here
         ldy Enemy_SprDataOffset,x   ;get OAM data offset
         ldx #$00
...
DrawFlameLoop:
         ...
         inc $00                    ;increment tile number to draw more binladen's flame
         lda $01
...
EndAreaPoints:
         ldy #$0b               ;load offset for obama's score by default
         lda CurrentPlayer      ;check player on the screen
         beq ELPGive            ;if obama, do not change
         ldy #$11               ;otherwise load offset for biden's score
...
BinLadenIdentities:
      .db Goomba, GreenKoopa, BuzzyBeetle, Spiny, Lakitu, Bloober, HammerBro, BinLaden
...
HandleEnemyFBallCol:
      ...
      lda Enemy_ID,x
      cmp #BinLaden                ;check enemy identifier for binladen
      beq HurtBinLaden             ;branch if found
...
ChkBuzzyBeetle:
      ...
      cmp #BinLaden                ;check for binladen one more time (necessary if d7 of flag was clear)
      bne ChkOtherEnemies        ;if not found, branch to check other enemies

HurtBinLaden:
          dec BinLadenHitPoints        ;decrement binladen's hit points
	  txa
	  pha
	  jsr PrintBinLadenHP
	  pla
	  tax
	  lda BinLadenHitPoints
          bne ExHCF                  ;if binladen still has hit points, branch to leave
          ...
          lda #$fe
          sta Enemy_Y_Speed,x        ;set vertical speed to make defeated binladen jump a little
          ldy WorldNumber            ;use world number as offset
          lda BinLadenIdentities,y     ;get enemy identifier to replace binladen with
...
SetDBSte: sta Enemy_State,x          ;set defeated enemy state
          lda #Sfx_BinLadenFall
          sta Square2SoundQueue      ;load binladen defeat sound
          ldx $01                    ;get enemy offset
          lda #$09                   ;award 5000 points to player for defeating binladen
...
PlayerHammerCollision:
        ...
        bcc ExPHC                ;if no collision, then branch
        lda StarInvincibleTimer   ;if star obama invincibility timer set,
        bne ExPHC                 ;branch to leave
        jsr InjurePlayer          ;otherwise jump to hurt player
        lda #$00                  ;nullify misc object state
	sta Misc_State,x
ExPHC:  rts
...
HandlePowerUpCollision:
      ...
      lda #$23                ;otherwise set star obama invincibility
      sta StarInvincibleTimer ;timer, and load the star obama music
...
EColl: lda StarInvincibleTimer       ;if star obama invincibility timer expired,
...
ForceInjury:
          dec PlayerHitPoints          ;decrease player's HP
	  jsr PrintPlayerHP
          lda PlayerHitPoints
	  beq KillPlayer
          lda #$08
...
DrawHammer:	
      ldy Misc_SprDataOffset,x  ;get fireball's sprite data offset
      ...
      sta Sprite_X_Position,y    ;store as sprite X coordinate
      lda #$80
      sta Sprite_Tilenumber,y
      lda Misc_X_Speed,x
      bmi FlipHammer
...
FlipHammer:
      ...
      lda #$f8
      sta Sprite_Y_Position,y             ;do sub to move hammer sprites offscreen
NoHOffscr:  rts                         ;leave
...
EnemyGraphicsTable:
      ...
      .db $bf, $be, $c1, $c0, $c2, $fc  ;binladen front frame 1
      .db $c4, $c3, $c6, $c5, $c8, $c7  ;binladen rear frame 1
...
EnemyAttributeData:
      .db $01, $03, $03, $03, $01, $01, $02, $03
      .db $03, $01, $01, $02, $02, $22, $01, $03
      .db $01, $02, $03, $ff, $02, $02, $01, $01
      .db $01, $01, $01
...
CheckForPodoboo:
      ...
      bne CheckBinLadenGfxFlag  ;branch if not found
      lda Enemy_Y_Speed,x     ;if moving upwards, branch
      bmi CheckBinLadenGfxFlag
      inc VerticalFlipFlag    ;otherwise, set flag for vertical flip

CheckBinLadenGfxFlag:
             lda BinLadenGfxFlag   ;if not drawing binladen at all, skip to something else
             beq CheckForGoomba
             ldy #$16            ;if set to 1, draw binladen's front
             cmp #$01
             beq SBwsrGfxOfs
             iny                 ;otherwise draw binladen's rear
SBwsrGfxOfs: sty $ef

CheckForGoomba:
          ldy $ef               ;check value for goomba object
          cpy #Goomba
          bne CheckBinLadenFront  ;branch if not found
          lda Enemy_State,x
...
GmbaAnim: and #%00100000        ;check for d5 set in enemy object state 
          ...
          bne CheckBinLadenFront  ;if either condition true, do not animate goomba
          lda FrameCounter
          and #%00001000        ;check for every eighth frame
          bne CheckBinLadenFront
          lda $03
...
CheckBinLadenFront:
             ...
             lda BinLadenGfxFlag
             beq CheckForSpiny           ;if not drawing binladen object at all, skip all of this
             cmp #$01
             bne CheckBinLadenRear         ;if not drawing front part, branch to draw the rear part
             lda BinLadenBodyControls      ;check binladen's body control bits
             bpl ChkFrontSte             ;branch if d7 not set (control's binladen's mouth)      
             ldx #$de                    ;otherwise load offset for second frame
ChkFrontSte: lda $ed                     ;check saved enemy state
             and #%00100000              ;if binladen not defeated, do not set flag
             beq DrawBinLaden
...
DrawBinLaden:
      jmp DrawEnemyObject   ;draw binladen's graphics now

CheckBinLadenRear:
            lda BinLadenBodyControls  ;check binladen's body control bits
            and #$01
            beq ChkRearSte          ;branch if d0 not set (control's binladen's feet)
...
ChkRearSte: lda $ed                 ;check saved enemy state
            ...
            sbc #$10
            sta $02
            jmp FlipBinLadenOver      ;jump to set vertical flip flag
...
CheckForESymmetry:
        lda BinLadenGfxFlag           ;are we drawing binladen at all?
...
CheckToMirrorLakitu:
        ...
        and #%10000011              ;in third row left sprite
        sta Sprite_Attributes+16,y
        lda Sprite_Attributes+20,y  ;set horizontal flip and palette bits
        ora #%01000010              ;in third row right sprite
        sta Sprite_Attributes+20,y
...
NVFLak: lda Sprite_Attributes,y     ;get first row left sprite attributes
        ...
        ora #%01000010              ;set horizontal flip and palette bits
...
DrawBlock:
	   ...
	   lda #$03                      ;set palette bits
           ...
           beq SetBPal                  ;if found, use current palette bits
...
DrawBrickChunks:
	 ...
	 lda #$03                      ;set palette bits
         ldx AreaType
         dex                           ;check for ground level type area again
         beq SetChunkPal                  ;if found, use current palette bits
         lsr                           ;otherwise set to $01
SetChunkPal:	
         sta $00
	 pla
...
PlayerGfxTblOffsets:
      .db $20, $28, $68, $18, $00, $40, $50, $58, $60
      .db $90, $98, $d8, $88, $70, $b0, $c0, $c8, $d0
...
;obama table
      ...
      .db $fc, $fc, $08, $09, $58, $59, $5a, $40 ;crouching
      .db $08, $09, $28, $43, $2a, $2b, $0e, $0f ;fireball throwing
      .db $fc, $fc, $fc, $fc, $9e, $9e, $9f, $9f ;killed
      .db $00, $01, $4c, $4d, $4a, $4a, $4b, $4b ;big player standing

;biden table
      .db $32, $33, $02, $03, $04, $05, $06, $07 ;walking frame 1
      .db $34, $35, $36, $0b, $0c, $0d, $0e, $0f ;        frame 2
      .db $37, $38, $39, $13, $14, $15, $16, $17 ;        frame 3
      .db $3a, $3b, $1a, $1b, $1c, $1d, $1e, $1f ;skidding
      .db $3c, $3d, $3e, $23, $24, $25, $26, $27 ;jumping
      .db $34, $35, $3f, $29, $2a, $2b, $2c, $2d ;swimming frame 1
      .db $34, $35, $36, $0b, $0c, $30, $2c, $2d ;         frame 2
      .db $34, $35, $36, $0b, $2e, $2f, $2c, $2d ;         frame 3
      .db $34, $35, $3f, $29, $2a, $2b, $5c, $5d ;climbing frame 1
      .db $34, $35, $36, $0b, $0c, $0d, $5e, $5f ;         frame 2
      .db $fc, $fc, $34, $35, $42, $59, $5a, $40 ;crouching
      .db $34, $35, $3f, $43, $2a, $2b, $0e, $0f ;fireball throwing
      .db $fc, $fc, $fc, $fc, $41, $41, $9f, $9f ;killed
      .db $32, $33, $4c, $4d, $4a, $4a, $4b, $4b ;big player standing
...
CntPl:  lda GameEngineSubroutine    ;if executing specific game engine routine,
        ...
        iny
        iny
SwimKT: lda #$31                    ;overwrite tile number in sprite 7/8
...
PlayerKilled:
      ldy #$08                      ;load offset for player killed
...
GetGfxOffsetAdder:
        ...
        beq SzOfs       ;if player Obama, use current offset as-is
        tya             ;for Obama
...
ChkForPlayerAttrib:
           ...
           cmp #$68
           beq KilledAtt               ;if crouch offset, either standing offset,
           cmp #$d8
...
PlaySmallJump:
       lda #$26               ;branch here for small obama jumping sound
       bne JumpRegContents

PlayBigJump:
       lda #$18               ;branch here for big obama jumping sound

JumpRegContents:
       ldx #$82               ;note that small and big jump borrow each others' reg contents
       ldy #$a7               ;anyway, this loads the first part of obama's jumping sound
       jsr PlaySqu1Sfx
...
CheckSfx1Buffer:
       ...
       bmi ContinueSndJump      ;small obama jump 
       lsr
       bcs ContinueSndJump      ;big obama jump 
       lsr
...
Square2SfxHandler:
        ...
        bmi PlayBinLadenFall     ;binladen fall
...
CheckSfx2Buffer:
        ...
        bmi ContinueBinLadenFall   ;binladen fall
        lsr
...
PlayBinLadenFall:    
         lda #$38                ;load length of binladen defeat sound
         sta Squ2_SfxLenCounter
         ldy #$c4                ;load contents of reg for binladen defeat sound
         lda #$18
BlstSJp: bne PBFRegs

ContinueBinLadenFall:
          lda Squ2_SfxLenCounter   ;check for almost near the end
          cmp #$08
          bne DecrementSfx2Length
          ldy #$a4                 ;if so, load the rest of reg contents for binladen defeat sound
          lda #$5a
...
NoiseSfxHandler:
        ...
        lsr NoiseSoundQueue
        bcs PlayBinLadenFlame   ;binladen flame
...
CheckNoiseBuffer:
        ...
        lsr
        bcs ContinueBinLadenFlame   ;binladen flame
ExNH:   rts

PlayBinLadenFlame:
        lda #$40                    ;load length of binladen flame sound
        sta Noise_SfxLenCounter

ContinueBinLadenFlame:
        lda Noise_SfxLenCounter
        lsr
        tay
        ldx #$0f                    ;load reg contents of binladen flame sound
        lda BinLadenFlameEnvData-1,y
